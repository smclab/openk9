# Helm package for Openk9 Gitlab Connector

## Introduction

This chart bootstraps Openk9 Gitlab Connector deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```bash
helm upgrade -i gitlab-connector openk9/openk9-gitlab-connector
```

The command deploys Openk9 Gitlab Connector on the Kubernetes cluster in the default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.
...
| ------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------- |
| `image.registry`    | Openk9 Gitlab Connector image registry                                                                                  | `REGISTRY_NAME`            |
| `image.repository`  | Openk9 Gitlab Connector image repository                                                                                | `REPOSITORY_NAME/openk9` |
| `image.digest`      | Openk9 Gitlab Connector image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag | `""`                       |
| `image.pullPolicy`  | Openk9 Gitlab Connector image pull policy                                                                               | `IfNotPresent`             |
| `image.pullSecrets` | Specify docker-registry secret names as an array                                                         | `[]`                       |
| `image.debug`       | Set to true if you would like to see extra information on logs                                           | `false`                    |

### Configure connections to other Openk9 services

Openk9 Gitlab Connector needs to communicate with other components to work. 
In particular Openk9 Gitlab Connector need to communicate with Ingestion to send data to it.
...
Openk9 Gitlab Connector doesn't need particular Service Account or rbac. 

But if you want for some reasons associate a pre-created Service Account to Openk9 Gitlab Connector you can do using following parameters:
...
| Name                                          | Description                                                                                | Value   |
| --------------------------------------------- | ------------------------------------------------------------------------------------------ | ------- |
| `serviceAccount.create`                       | Enable creation of ServiceAccount for Gitlab Connector pods                                        | `true`  |
...
Openk9 Gitlab Connector can openk9 Apis externally.
...
Openk9 Gitlab Connector supports Readiness, Liveness and Startup Probes.
...
Openk9 Gitlab Connector chart allow setting resource requests and limits for all containers inside the chart deployment. These are inside the `resources` value (check parameter table). Setting requests is essential for production workloads and these should be adapted to your specific use case.
...
| Name                | Description                                                                                              | Value                      |
| ------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------- |
| `podSecurityContext.enabled`                        | Enable Gitlab Connector pods' Security Context                                                                                                                                                                            | `true`           |
| `podSecurityContext.fsGroupChangePolicy`            | Set filesystem group change policy                                                                                                                                                                                | `Always`         |
| `podSecurityContext.sysctls`                        | Set kernel settings using the sysctl interface                                                                                                                                                                    | `[]`             |
| `podSecurityContext.supplementalGroups`             | Set filesystem extra groups                                                                                                                                                                                       | `[]`             |
| `podSecurityContext.fsGroup`                        | Set Gitlab Connector pod's Security Context fsGroup                                                                                                                                                                       | `1001`           |
| `podSecurityContext.enabled`                  | Enabled Gitlab Connector containers' Security Context                                                                                                                                                                     | `true`           |
| `podSecurityContext.runAsUser`                | Set Gitlab Connector containers' Security Context runAsUser                                                                                                                                                               | `1001`           |
| `podSecurityContext.runAsGroup`               | Set Gitlab Connector containers' Security Context runAsGroup                                                                                                                                                              | `1001`           |
| `podSecurityContext.runAsNonRoot`             | Set Gitlab Connector container's Security Context runAsNonRoot                                                                                                                                                            | `true`           |
