...
package io.openk9.searcher.filter;
...
public class RouteFilters {
	...
	/** Key under which {@link #tenantIdRouteFilter} stores the resolved tenantId on the {@link RoutingContext}. */
	public static final String TENANT_ID_KEY = "_tenantId";
	...
	void tenantIdRouteFilter(RoutingContext rc) {
		...
		if (tenantId != null) {
			if (log.isDebugEnabled()) {
				log.debugf("Setting '%s' to %s in routingContext", TENANT_ID_KEY, tenantId);
			}

			rc.put(TENANT_ID_KEY, tenantId);
		}
		else {
			if (log.isDebugEnabled()) {
				log.warnf(
