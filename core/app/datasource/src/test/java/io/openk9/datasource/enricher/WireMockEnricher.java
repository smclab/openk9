...
/**
 * WireMock server simulating an enricher's HTTP endpoints.
 */
public class WireMockEnricher
	implements QuarkusTestResourceLifecycleManager {

	public static final String FORM_JSON_FILE =
		"/enricher/form.json";
	...
	private void stubForm() {
		try (InputStream is =
			TestUtils.getResourceAsStream(FORM_JSON_FILE)) {

			String form = new String(is.readAllBytes());

			wireMockServer.stubFor(WireMock
				.get(urlEqualTo(
					HttpDatasourceServiceClient.FORM_PATH))
				.willReturn(okJson(form))
			);
		}
		catch (IOException e) {
			fail("Could not configure Wiremock server."
