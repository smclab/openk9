	...
	public enum UserField {
		...
		NAME_SURNAME {
			...
			public void apply(
				...
				String name_surname = jwt.getGivenName() + " " + jwt.getFamilyName();
		...
		private static void apply(
			...
			if (value != null && !value.isBlank()) {
				if (docTypeField.isKeyword()) {
					boolQueryBuilder.should(
						QueryBuilders.termQuery(docTypeField.getFieldName(), value));
				}
				else {
					boolQueryBuilder.should(
						QueryBuilders.matchQuery(docTypeField.getFieldName(), value));
				}
			}
		...
		private static void apply(
			...
			if (values != null && !values.isEmpty()) {
				if (docTypeField.isKeyword()) {
					boolQueryBuilder.should(
						QueryBuilders.termQuery(docTypeField.getFieldName(), values));
				}
				else {
					boolQueryBuilder.should(
						QueryBuilders.matchQuery(docTypeField.getFieldName(), values));
				}
			}
