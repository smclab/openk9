# Helm package for Openk9 Agentic Rag Module

## Introduction

This chart bootstraps Openk9 Agentic Rag Module deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```bash
helm upgrade -i agentic-rag-module openk9/openk9-agentic-rag-module
```

The command deploys Openk9 Agentic Rag Module on the Kubernetes cluster in the default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.
...
| ------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------- |
| `image.registry`    | Openk9 Agentic Rag Module image registry                                                                                  | `REGISTRY_NAME`            |
| `image.repository`  | Openk9 Agentic Rag Module image repository                                                                                | `REPOSITORY_NAME/openk9` |
| `image.digest`      | Openk9 Agentic Rag Module image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag | `""`                       |
| `image.pullPolicy`  | Openk9 Agentic Rag Module image pull policy                                                                               | `IfNotPresent`             |
...
Openk9 Agentic Rag Module service handles retention mechanism to handle saved conversations. Following parameters allow to handle in better possible way.
...
Openk9 Agentic Rag Module service handles possibility to load documents from external and use during chat. Following parameters allow to handle in better possible way.
...
In particular Openk9 Agentic Rag Module need to communicate with Datasource and Tenant Manager.
...
### Configure admin password (Security)

Openk9 Agentic Rag Module requires an admin password to protect privileged endpoints. The password is injected as the `OPENK9_SECURITY_ADMIN_PASSWORD` environment variable, read from a Kubernetes Secret.
...
Openk9 Agentic Rag Module supports collecting metrics e tracing about traffic using [Phoenix Arize](https://phoenix.arize.com/).
...
Openk9 Agentic Rag Module doesn't need particular Service Account or rbac. 

But if you want for some reasons associate a pre-created Service Account to Openk9 Agentic Rag Module you can do using following parameters:
...
Openk9 Agentic Rag Module doesn't need to openk9 Apis externally.
...
Openk9 Agentic Rag Module supports Readiness, Liveness and Startup Probes.
...
Openk9 Agentic Rag Module chart allow setting resource requests and limits for all containers inside the chart deployment. These are inside the `resources` value (check parameter table). Setting requests is essential for production workloads and these should be adapted to your specific use case.
