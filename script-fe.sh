...
while IFS= read -r NAME || [[ -n "$NAME" ]]; do
    ...
    docker build -t $NAME:3.0.2-SNAPSHOT -f js-packages/$NAME/Dockerfile .
    
    # Command 2 (Example: sudo apt install "$NAME")
    docker tag $NAME:3.0.2-SNAPSHOT registry.smc.it:49083/openk9/openk9-$NAME:3.0.2-SNAPSHOT
