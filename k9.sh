...
#
# k9.sh — OpenK9 local development CLI (3.0.x maintenance branch)
#
...
#
# Differences vs main:
#   - No `api-gateway` Spring Boot module on this branch — the simple
#     Caddy reverse proxy fronts the backend services directly.
#   - Keycloak is part of the core compose, not an opt-in profile.
#   - AI modules limited to rag-module, embedding-module, talk-to.
#
# Prerequisites:
...
# On 3.0.x Keycloak is a core dependency, so there is no `oauth2`
# overlay. Unknown profiles (including `oauth2`) are rejected by
# parse_args via profile_to_file returning a non-zero exit code.
...
VALID_PROFILES="core file-handling gen-ai all"

# --- Profile to services mapping ---
# `api-gateway` is intentionally absent: the module does not exist on
# the 3.0.x branch.
    ...
    for arg in "$@"; do
        if [ "$arg" == "--build" ] || [ "$arg" == "-b" ]; then
            ...
            if profile_to_file "$p" >/dev/null; then
                ...
                if [ "$p" = "oauth2" ]; then
                    echo "Note: on 3.0.x Keycloak is part of the core stack;" \
                         "the 'oauth2' profile is not supported."
                fi
                exit 1
            fi
        elif [[ "$arg" == -* ]]; then
...
build_gen_ai() {
    ...
    docker build --pull --platform "$JIB_PLATFORM" -t "$GROUP/openk9-embedding-module-base:$TAG" -f ai-packages/embedding-modules/Dockerfile.base ai-packages/embedding-modules
...
_has_java_service() {
    local java_services="tenant-manager datasource ingestion searcher file-manager tika"
    for svc in "$@"; do
...
usage() {
    cat <<'USAGE'
k9.sh — OpenK9 local development CLI (3.0.x)
...
Options:
  ...
  --tag=TAG              Docker image tag (default: 3.0.x-SNAPSHOT)
...
Profiles (--with):
  ...
  Note: unlike main, there is no `oauth2` profile on this branch.
  Keycloak is a core dependency and is always started.
...
Services (for targeted build/restart):
  ...
  Note: unlike main, there is no `api-gateway` service on this
  branch — the module does not exist on 3.0.x and Caddy fronts
  the backend services directly.
...
Startup behavior:
  ...
  seed.js, which idempotently provisions:
    ...
    - The Keycloak realm and OAuth2 client for the demo tenant
